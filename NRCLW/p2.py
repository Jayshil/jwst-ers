...
all_lc, all_lc_var = dataset1['spectra'], dataset1['variance']
times = dataset1['times']
all_lc_err = np.sqrt(all_lc_var)

## Spectroscopic lightcurve analysis
f16 = Path(os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/wavelength_ch.dat')
f17 = Path(os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/spectroscopic_lc_ch_' + str(chanel_nos) + '.pkl')
if f16.exists() and f17.exists():
    ...
    pickle.dump(spectral_lcs, open(os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/spectroscopic_lc_ch_' + str(chanel_nos) + '.pkl','wb'))

    ## Saving wavelengths and wavelength bins
    f12 = open(os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/wavelength_ch.dat', 'w')
...
def fit_lc(lightcurves, ch_name):
    ...
    print('')
    # Output folder
    pout = os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/' + ch_name
...
for i in tqdm(range(spec_lc1.shape[1])):
    pout = os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/CH' + str(i)
...
plt.xscale('log')
plt.yscale('log')
plt.savefig(os.getcwd() + '/NRCLW/Analysis/' + analysis1 + '/alan_deviation.png')
plt.close(fig)